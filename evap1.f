      ...
      double precision :: hbase,czero,length
      double precision :: Ma, del,Db,B,A,epsi,Pe,theta,tri,kd
      end module parameters

      program driver
      use parameters
      implicit double precision (a-h,o-z)
      INTEGER MXNPDE, MXNEQ, MXLIW, MXLRW, MXNRWK,MAXORD,npde,n,neq
      integer nx,ny,maxn
c m is the vertical number of pts, n is horizontal number of points
      PARAMETER (MXNEQ=50000,npde=3,n=1200,MAXORD=5)
      parameter(neq=n*npde)
      PARAMETER (MXLIW =20+MXNEQ,ML=4*npde,MU=4*npde)
c matrix is banded dont get rid of these, alt h and c
      parameter(mxlrw=40+(MAXORD+4)*MXNEQ+(2*ML+MU+1)
     + *MXNEQ+2*(MXNEQ/(ML+MU+1)+1))
      INTEGER NPTS, NPRINT
c nprint controls outputting
      parameter(nprint=100)
	  
c stuff for dassl
      INTEGER INFO(15), IWORK(MXLIW), IPAR(2),icount,k,j,ii,kk,jj
      DOUBLE PRECISION Y(neq), YPRIME(NEQ), RTOL(1), ATOL(1),
     +     RWORK(MXLRW), RWK,rpar(1),depth,pi
     +     ,tfinal,xj,tout,t
     +     ,massh,massg,massc(n),massctot,runmassh(nprint),
     +     runmassg(nprint),
     +     runmassc(nprint),prec
c stuff for printing as well    
      double precision h(n*nprint),g(n*nprint)
      DOUBLE PRECISION  T0, TE,TPRINT(nprint),kay
      EXTERNAL RESID, DDASSL
      INTEGER I,IBAND,IDID,IPRINT,IRES,LIW,LRW,NCTF,
     +         NETF,NFCN,NOINV,NRWK,NSTEPS,ioff
      INTEGER mycount
      double precision x(n)
      double precision hmax,edge,conserv,conserv1
...
c set x and y vectors here 
      dx=length/dble(n-1)
      do i=1,n
           x(i)=dble(i-1)*dx-length/2d0
c           x(i)=dble(i-1)*dx
      enddo

	   
c write the data files	   
      open(23,file='parms.dat',status='unknown')
      open(24,file='profile.dat',status='unknown')
      open(25,file='edges.dat',status='unknown')
      open(45,file='initial.dat',status='unknown')	   	  
      open(46,file='cons.dat',status='unknown')	   	  
c      hbase=1d-3
      hbase=(tri*A/theta)**(1d0/3d0)
      epsi = 1d1*hbase
      lzero=0d0
c      czero=0d0
      czero=0.2d0
...
c initial condition	  
      do k=1,n
           if (ABS(x(k)).lt.1d0) then
           y(npde*k-2)=2d0*(1d0-x(k)**2d0)+hbase
c           y(npde*k-1)=2d-1*((1d0-x(k)**2d0)**2)
           y(npde*k-1)=czero
           else
           y(npde*k-2)=hbase
           y(npde*k-1)=0d0
           endif
c           y(npde*k-2)=1d0-sin(x(k))*hbase
           y(npde*k)=lzero
c           y(npde*k-1)= 2d-1*tanh((y(npde*k-2)-hbase)/epsi)

      enddo

             conserv=y(1)*y(2)+y(3*n-2)*y(3*n-1)
            do kk=2,n-1
             conserv=conserv+2d0*y(npde*kk-2)*y(npde*kk-1)
            enddo
              conserv=dx*conserv/2d0

             conserv1=y(1)+y(3*n-2)
            do kk=2,n-1
             conserv1=conserv1+2d0*y(npde*kk-2)
            enddo
              conserv1=dx*conserv1/2d0

c write initial condition and parameters
      write(45,*) sngl(x),sngl(y)
      write(23,*) hbase,n,dx,tfinal,nprint,Pe,A,length
      close(45)
      close(23)
      write(*,*)sngl(conserv)
c   dassl stuff
...
C    Banded Jacobian
      iband=npde*4 !!SETS BOUNDING ON EITHER SIDE
      INFO(6) = 1
      IWORK(1) = IBAND
      IWORK(2) = IBAND	  
...
C Check length arrays - sanity checks
...
c write the h profile
         do 105 kk=1,n
             write(24,*) TOUT,sngl(x(kk)),sngl(y(npde*kk-2))
     +        ,sngl(y(npde*kk-1)),sngl(y(npde*kk))
...
c helpful for finding edges
             do kk=n,1,-1
                if(y(npde*kk-2).gt.hbase)then
c crude interpolation
c                   edge=(x(kk)+x(kk+1))/2d0
c proper linear interpolation
                   edge=x(kk)+dx*(hbase-y(npde*kk-2))
     +       /(y(npde*kk+1)-y(npde*kk-2))
                   kedge=kk
                   goto 99
                endif
             enddo
...
c find the max value
             hmax=0d0
            do kk=1,n
             if(y(npde*kk-2).gt.hmax)then
              hmax=y(npde*kk-2)
             endif
            enddo
...
!! writing data here 
       write(25,*)sngl(t),sngl(edge),sngl(hmax),sngl(conserv1)
       write(46,*)sngl(t),sngl(conserv)
       write(*,*)sngl(t),sngl(edge),sngl(hmax),sngl(conserv)
...
c if arrays are allocatable then defer the size of them 
      SUBROUTINE RESID(T, Y, YPRIME, DELTA, IRES, RPAR, IPAR)
      use parameters
      implicit none
      INTEGER IRES,IPAR(2),n,npde
      INTEGER i,j,k,ii,jj
      DOUBLE PRECISION t
      DOUBLE PRECISION Y(*), YPRIME(*), DELTA(*), RPAR
      double precision, allocatable :: h1(:),c(:),lambda(:),h1x(:)
     +,hubar(:),dflux(:),h1xx(:),h1xxx(:),x(:),f(:)
      double precision, allocatable :: nu(:),press(:)
     +,bigPi(:),px(:),bigJ(:),Jhx(:),ubar(:)
      double precision, allocatable :: cvx(:),lx(:),hcx(:)
     +,convec(:),struct(:), destruct(:),cflux(:),diffc(:)
     +,convec1(:),cflux1(:),eflux(:)
 
      n=ipar(1)
      npde=ipar(2)

      allocate (h1(n),c(n),lambda(n),h1x(n),hubar(n)
     +,dflux(n),h1xx(n),h1xxx(n),f(n)
     +,x(n)
     +,nu(n),press(n),bigPi(n)
     +,px(n),bigJ(n),Jhx(n),ubar(n)
     +,lx(n),cvx(n),hcx(n)
     +,convec(n),struct(n), destruct(n), cflux(n)
     +,convec1(n),cflux1(n),diffc(n),eflux(n))
...
c set central differences here
c check these
         do ii=3,n-2
            h1xxx(ii)=(h1(ii+2)-h1(ii-2)-2d0*(h1(ii+1)-h1(ii-1)))/
     + (2d0*(dx**3d0))
            h1xx(ii)=(h1(ii+1)-2d0*h1(ii)+h1(ii-1))/(dx**2d0)
            h1x(ii)=(h1(ii+1)-h1(ii-1))/(2d0*dx)
         enddo
         h1xxx(1)=0d0
         h1xxx(n)=0d0
         h1xxx(2)=(h1(4)-h1(2)-2d0*(h1(3)-h1(1)))/
     + (2d0*dx**3d0)
         h1xxx(n-1)=(h1(n-1)-h1(n-3)-2d0*(h1(n)-h1(n-2)))/
     + (2d0*dx**3d0)
            h1x(1)=0d0
            h1x(n)=0d0
         ...
         ii=n-1
            ...
            h1xx(1)=2d0*(h1(2)-h1(1))/dx**2d0
            h1xx(n)=2d0*(h1(n-1)-h1(n))/dx**2d0

c set viscosity, mu is used by dassl, use nu
      do i=1,n
        nu(i)=1d0 + del*lambda(i)
      enddo 

c set disjoining pressure
      do ii=1,n
	  bigPi(ii)=A/(h1(ii)**3d0)
      enddo 

c set pressure
      do i=1,n
       press(i)=-h1xx(i)-bigPi(i)
      enddo 

c set p_x (negative of px)
      do i=1,n
        px(i)=-h1xxx(i)+(3d0*A*h1x(i)/(h1(i)**4d0))
      enddo 
c set evaporative flux
      do i=1,n
        eflux(i)=(theta + (tri*press(i)))/(kd + h1(i))
      enddo


c set ubar  
      do i=1,n 
        ubar(i)=-px(i)*(h1(i)**2d0)/3d0/nu(i)
      enddo
...
c bring it all together 
      do ii=2,n-1
        dflux(ii)=((hubar(ii+1)-hubar(ii-1))/(2d0*dx))+eflux(ii)
      enddo
c boundary conditions for h 
 
c      dflux(1)=(hubar(2)-hubar(1))/(dx)
c      dflux(n)=(hubar(n)-hubar(n-1))/(dx)
        dflux(1)=0d0
        dflux(n)=0d0

c this is now for concentration, cross x avg

c setup (c)x
        cvx(1)=0d0
        cvx(n)=0d0
      do i=2,n-1
       cvx(i)=(c(i+1)-c(i-1))/(2d0*dx)
      enddo


c this is now for lambda, cross x avg

c setup (lambda)x
        lx(1)=0d0
        lx(n)=0d0
      do i=2,n-1
       lx(i)=(lambda(i+1)-lambda(i-1))/(2d0*dx)
      enddo

c set (h c_x)_x
      do i=2,n-1
c       hcx(i)=(h1(i+1)*cvx(i+1)-h1(i-1)*cvx(i-1))/(2d0*dx)
        hcx(i)=h1x(i)*cvx(i)
     +   +h1(i)*(c(i+1)-2d0*c(i)+c(i-1))/(dx)**2d0
      enddo
        hcx(1)=2d0*h1(i)*(c(2)-c(1))/(dx)**2d0
        hcx(n)=2d0*h1(i)*(c(n-1)-c(n))/(dx)**2d0

c bring together advective terms
      do i=1,n
       convec1(i)=ubar(i)*cvx(i)
      enddo

      do j=1,n
       diffc(j)=hcx(j)/(h1(j)*Pe) + (c(j)*eflux(j))/h1(j)
      enddo


c bring together advective terms
      do i=1,n
       convec(i)=ubar(i)*lx(i)
      enddo

      do j=1,n
       f(j) = tanh((h1(j)-hbase)/epsi)
c       struct(j)=(f(j)*(1-lambda(j)))/Db
c       struct(j)=f(j)*(1-lambda(j))*((1 + c(j))**2d0)/Db
       struct(j)=(1-lambda(j))*((1 + c(j))**2)/Db
      enddo

      do j=1,n
       destruct(j)=B*lambda(j)*h1(j)*ABS(h1xxx(j))/(nu(j))
      enddo

      do i=1,n
       cflux1(i)=convec1(i)-diffc(i)
      enddo
c        cflux(1)=0d0
c        cflux(n)=0d0
      ...
      do i=1,n
       cflux(i)=convec(i)-struct(i) + destruct(i)
      enddo
	   
      do k=1,n
      delta(npde*k-2)=yprime(npde*k-2)+dflux(k)
      delta(npde*k-1)=yprime(npde*k-1)+cflux1(k)
      delta(npde*k)=yprime(npde*k)+cflux(k)	   
      enddo
...
C
C     CHECK FOR TOO MANY STEPS
      IF((IWORK(LNST)-IWORK(LNSTL)).LT.50000)
     *   GO TO 510
           IDID=-1
           GO TO 527
C
...
610   WRITE (XERN3, '(1P,D15.6)') TN
      CALL XERMSG ('SLATEC', 'DDASSL',
     *   'AT CURRENT T = ' // XERN3 // ' 50000 STEPS TAKEN ON THIS ' //
